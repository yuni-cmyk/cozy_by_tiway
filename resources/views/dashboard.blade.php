...
<div class="max-w-xl mx-auto mt-10 bg-purple-100 p-6 rounded-xl shadow">
    ...
    <form action="{{ route('dashboard.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            <label class="block font-semibold text-violet-700">Nama Pelanggan</label>
            <input type="text" name="nama_pelanggan" required class="w-full p-2 border border-pink-300 rounded">
        </div>

        <div>
            <label class="block font-semibold text-violet-700">Email</label>
            <input type="email" name="email" required class="w-full p-2 border border-pink-300 rounded">
        </div>

        <div>
            <label class="block font-semibold text-violet-700">No Telepon</label>
            <input type="text" name="no_telepon" required class="w-full p-2 border border-pink-300 rounded">
        </div>

        <div>
            <label class="block font-semibold text-violet-700">Alamat</label>
            <textarea name="alamat" required class="w-full p-2 border border-pink-300 rounded"></textarea>
        </div>

        <button type="submit" class="bg-pastelpurple-500 hover:bg-lavender-600 text-purple-900 font-bold py-2 px-4 rounded w-full">
            Simpan
        </button>
    </form>
</div>
